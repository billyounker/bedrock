# bedrock_test Deployment

- [bedrock_test Deployment](#bedrock-deployment)
  - [Setup](#setup)
...
- Use `gcloud config configurations create bedrock_test` and authorize Google Cloud `gcloud auth login`
...
```bash
gcloud projects create bedrock_test-staging --name="bedrock_test Staging"
...
```bash
./deployment/scripts/provision_gcloud staging bedrock_test-staging
```
