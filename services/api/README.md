...
![Run Tests](https://github.com/billyounker/bedrock/workflows/Tests/badge.svg)
...
- `MONGO_URI` - MongoDB URI to connect to, defaults to `mongodb://localhost/bedrock_test_dev`
...
- `ADMIN_EMAIL` - Default dashboard admin user `bill.younker@gmail.com`
- `ADMIN_PASSWORD` - Default dashboard admin password `[change me]`
- `APP_NAME` - Default product name to be used in emails `bedrock_test`
- `APP_URL` - URL for app defaults to `http://localhost:2200`
- `POSTMARK_FROM` - Reply email address `no-reply@bedrock-test.netlify.app`
...
```
docker build -t bedrock_test-api .
```
...
```
docker build -t bedrock_test-api-jobs -f Dockerfile.jobs .
```
